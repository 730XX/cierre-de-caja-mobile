...
public class MainActivity extends AppCompatActivity {
    ...
    private void initCardIngresosPropinas() {
        ...
        // Datos estáticos de ejemplo - con datos para mostrar el footer
        List<ItemTablaFila> datosPropinas = new ArrayList<>();
        datosPropinas.add(new ItemTablaFila("POS", "S/30.00", true));

        cardIngresosPropinas.setTablaData(datosPropinas);
    ...
    private void initCardIngresosCreditos() {
        ...
        cardIngresosCreditos.setTablaData(datosCreditos);
    ...
    private void setupBarChart(List<ItemCategoria> categorias) {
        ...
        barData.setBarWidth(0.15f); // Barras más angostas = más espacio entre ellas

        // Configuración del gráfico ANTES de setData
        barChartCategorias.setFitBars(false); // Desactivar fitBars para controlar manualmente el rango
        ...
        barChartCategorias.setExtraLeftOffset(-30f); // Reducir espacio izquierdo
        barChartCategorias.setExtraRightOffset(-30f); // Reducir espacio derecho

        // Eje X - deshabilitado, los valores se muestran desde el dataSet
        XAxis xAxis = barChartCategorias.getXAxis();
        xAxis.setPosition(XAxis.XAxisPosition.BOTTOM);
        xAxis.setDrawGridLines(false);
        xAxis.setDrawAxisLine(false);
        xAxis.setDrawLabels(false); // No dibujar labels del eje X
        xAxis.setAxisMinimum(-0.5f); // Acercar las barras al borde izquierdo
        xAxis.setAxisMaximum(categorias.size() - 0.5f); // Acercar las barras al borde derecho
        xAxis.setCenterAxisLabels(false);

        // Eje Y izquierdo - configurar rango de 0 a 100
